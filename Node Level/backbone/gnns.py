...
class GIN_Increment(nn.Module):
    def __init__(self,
                 args,):
        super(GIN_Increment, self).__init__()
        ...
        #     lin = torch.nn.Linear(dims[l], dims[l+1])
        #     self.gat_layers.append(GINConv(lin, 'sum'))
        lin = torch.nn.Linear(dims[0], dims[1])
        self.conv1 = GINConv(lin, 'sum')
        lin = torch.nn.Linear(dims[1], dims[1])
        self.conv2 = GINConv(lin, 'sum')
    ...
    def forward_batch(self, blocks, features):
        ...
        h, e = self.gat_layers[0].forward_batch(blocks[0], features)
        x = F.relu(h)
        emb, e = self.gat_layers[1].forward_batch(blocks[1], x)
        # self.second_last_h = logits if len(self.gat_layers) == 1 else h
        e_list = e_list + e
        logits = self.fc(emb)
        return logits, e_list
...
class GAT_Increment(nn.Module):
    def __init__(self,
                 args,
                 heads,
                 activation):
        super(GAT_Increment, self).__init__()
        #self.g = g
        self.num_layers = args.GAT_args['num_layers']
        self.gat_layers = nn.ModuleList()
        self.norm_layers = nn.ModuleList()
